---
title: "lab 7 Spatial Point Pattern Analysis"
author: "Iris Foxfoot"
date: "2/17/2021"
...
#get hoboldt county outline
humboldt <- read_sf(dsn = here("redtreevoledata"), layer = "california_county_shape_file") %>% 
...
```{r}
voles_sp <- as(voles, "Spatial") # convert to spatial

proj4string(voles_sp) <- "+proj=utm +zone=10 +datum=WGS84 +units=m +no_defs +ellps=WGS84 +towgs84=0,0,0" #add UTM projections so it's 2D

voles_ppp <- as(voles_sp, "ppp") #converto to spatial point pattern
...
#Combine as a point patter object (points + window)
